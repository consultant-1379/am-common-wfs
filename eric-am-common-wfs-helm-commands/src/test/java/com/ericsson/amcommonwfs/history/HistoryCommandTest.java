...
@SpringBootTest(classes = {HistoryCommand.class, TemporaryFileServiceImpl.class})
...
public class HistoryCommandTest {
    ...
    @Test
    public void verifyHistoryCommandStructureWithClusterConfig() {
        ...
        historyCommand.createCommand(execution);
        StringBuilder command = (StringBuilder) execution.getVariable(COMMAND);
        assertThat(command.toString()).matches("helm history my-release --max 2147483647 -o json --namespace my-namespace --kubeconfig "
                + clusterConfig);
    }

    @Test
    public void verifyHistoryCommandStructureWithClusterConfigFile() {
        String clusterConfig = "test01.config";
        execution.setVariable(CLUSTER_NAME, clusterConfig);
        execution.setVariable(RELEASE_NAME, "my-release");
        execution.setVariable(NAMESPACE, "my-namespace");
        ...
        historyCommand.createCommand(execution);
        StringBuilder command = (StringBuilder) execution.getVariable(COMMAND);
        assertEquals(command.toString(), "helm history my-release --max 2147483647 -o json --namespace my-namespace --kubeconfig "
                + execution.getVariable(CLUSTER_NAME));
    }

    @Test
    public void verifyHistoryCommandStructureWithNonDefaultHelmClientVersion() {
        execution.setVariable(RELEASE_NAME, "my-release");
        execution.setVariable(NAMESPACE, "my-namespace");
        execution.setVariableLocal(HELM_CLIENT_VERSION, HELM_COMMAND_3_10);
        historyCommand.createCommand(execution);
        StringBuilder command = (StringBuilder) execution.getVariable(COMMAND);
        assertThat(command.toString()).matches("helm-3.10 history my-release --max 2147483647 -o json --namespace my-namespace --kubeconfig " + execution.getVariable(CLUSTER_NAME));
    }
}
