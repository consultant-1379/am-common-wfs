...
Contract.make {
    description("""
Represents a failed scenario of scaling down an instantiated resource with a blank namespace in message body

```
given:
  client requests to scale a resource down
when:
  invalid scale down request is submitted with a blank namespace in message body
then:
