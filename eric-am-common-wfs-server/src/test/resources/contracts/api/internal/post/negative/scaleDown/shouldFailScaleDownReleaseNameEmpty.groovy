...
Contract.make {
    description("""
Represents a failed scenario of scaling down an instantiated resource with a blank releaseName in message body

```
given:
  client requests to scale a resource down
when:
  an invalid scale down request is submitted with empty releaseName in message body
then:
