...
when:
  kind not provided in cluster config file
then:
...
""")
    request {
        ...
        multipart(
                clusterConfig: named(
                        name: $(consumer(regex('.+nullKind.config')),
                                producer('cluster01nullKind.config')),
                        content: $(consumer(regex(nonEmpty())),
                                producer(file('withoutKind.config')))
                )
        )
        headers {
        ...
        body("""
            {
                "errorDetails":[
                 {
                    "parameterName":"kind",
                    "message":"kube config kind cannot be null"
                 }
            ]
        }
        """
