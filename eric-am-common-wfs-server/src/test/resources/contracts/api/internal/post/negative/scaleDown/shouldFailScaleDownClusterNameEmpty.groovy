...
Contract.make {
    description("""
Represents a failed scenario of scaling down an instantiated resource with a blank clusterName in message body

```
given:
  client requests to scale a resource down
when:
  an invalid scale down request is submitted with empty clusterName in message body
then:
