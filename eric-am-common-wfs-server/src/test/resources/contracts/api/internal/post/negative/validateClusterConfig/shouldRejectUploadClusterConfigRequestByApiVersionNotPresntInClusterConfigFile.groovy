...
when:
  apiVersion details not provided in cluster config file
then:
...
""")
    request {
        ...
        multipart(
                clusterConfig: named(
                        name: $(consumer(regex('.+invalidApi\\.config')),
                                producer('cluster01invalidApi.config')),
                        content: $(consumer(regex(nonEmpty())),
                                producer(file('withoutApiVersion.config')))
                )
        )
        headers {
                ...
                "errorDetails":[
                 {
                    ...
                    "message":"kube config api version cannot be null"
                 }
            ]
        }
        """
