...
when:
  valid patch secret request is submitted with empty secret value
then:
...
""")
    request {
        ...
        multipart(
                json: $(c(regex(file('jsonBodyRegex/empty-secret.txt').asString())), p(file('requestBody/empty-secret.json'))),
                clusterConfig: named(
