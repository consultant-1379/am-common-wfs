...
Contract.make {
  description("""
Represents a failed scenario of patching a secret with a missing secret value in message body

```
given:
  client requests to patch a secret
when:
  invalid patch secret request is submitted with a missing secret value in message body
then:
...
""")
    ...
    response {
        status BAD_REQUEST()
        body(file("errorResponses/missingSecretValue.json"))
        headers {
            contentType(applicationJson())
        }
    }
