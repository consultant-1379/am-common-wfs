...
Contract.make {
  description("""
Represents a failed scenario of patching a secret with a missing namespace and secret key in message body

```
given:
  client requests to patch a secret
when:
  invalid patch secret request is submitted with a missing namespace and secret key in message body
then:
...
""")
    ...
    response {
        status BAD_REQUEST()
        body(file("errorResponses/missingNamespaceAndSecretKey.json"))
        headers {
            contentType(applicationJson())
        }
    }
