...
when:
  valid patch secret request is submitted without a cluster name
then:
...
""")
    request {
        ...
        multipart(
                json: $(c(regex(file('jsonBodyRegex/no-cluster-name.txt').asString())), p(file('requestBody/no-cluster-name.json'))),
                clusterConfig: named(
