...
with cluster config file provided

```
given:
  client requests to get all secret in a namespace
when:
  invalid get request is submitted without namespace
then:
...
""")
    request {
        method 'POST'
        url'/api/internal/kubernetes/secrets'
        headers {
    ...
    response {
        status BAD_REQUEST()
        body (file("namespaceNotPresentErrorMessage.json"))
        headers {
