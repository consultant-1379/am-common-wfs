...
Contract.make {
    description("""
Represents an unsuccessful scenario of scaling an application with an empty values.yaml file

```
given:
  client requests to scale a resource
when:
  a scale request with an empty values.yaml file is submitted
then:
