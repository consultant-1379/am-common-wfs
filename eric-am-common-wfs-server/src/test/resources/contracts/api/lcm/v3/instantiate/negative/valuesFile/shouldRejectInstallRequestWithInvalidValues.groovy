...
Contract.make {
    description("""
Represents an unsuccessful scenario of instantiating an application with an invalid values.yaml file

```
given:
  client requests to instantiate a resource
when:
  an instantiate request with an invalid values.yaml file is submitted
then:
