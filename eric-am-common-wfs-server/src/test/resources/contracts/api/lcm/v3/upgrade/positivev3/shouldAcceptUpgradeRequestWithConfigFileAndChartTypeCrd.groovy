...
Contract.make {
    description("""
Represents a successful scenario of upgrading an application with a cluster01.config file and a CRD resource

```
given:
  client requests to upgrade a resource with a cluster01.config file
when:
  valid upgrade request submitted with chartType = CRD
then:
...
""")
    request {
        ...
        multipart(
                json: $(c(regex(nonEmpty())), p(file('multipart-json-part-with-additional-params-chartType.json'))),
