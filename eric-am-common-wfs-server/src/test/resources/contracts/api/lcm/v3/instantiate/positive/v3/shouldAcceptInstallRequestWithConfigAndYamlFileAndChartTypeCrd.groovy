...
Contract.make {
    description("""
Represents a successful scenario of instantiating an application with a values.yaml and config file and a CRD resource

```
given:
  client requests to instantiate a resource with a values.yaml and cluster01.config files and a CRD resource
when:
  valid instantiate request submitted with chartType = CRD
then:
