 ...
 ******************************************************************************/
package contracts.api.lcm.v3.terminate.positivev3.clusterconfig

import org.springframework.cloud.contract.spec.Contract

Contract.make {
    description("""
Represents a successful scenario of terminating an Application with cluster config file

```
given:
  client requests to terminate a resource
when:
  the release name and the cluster name are valid
then:
...
"""
    ...
    request {
        ...
        urlPath($(regex("/api/lcm/v3/resources/[a-z]([-a-z0-9]*[a-z0-9])?/terminate"))) {
            queryParameters {
                parameter 'clusterName': value(consumer(regex("[a-zA-Z0-9]+(\\.config)?+")), producer("mycluster"))
    ...
    response {
        ...
        body(
                """
                {
                    ...
                    "links": {
                    "self": "http://localhost/${fromRequest().path(0)}/${fromRequest().path(1)}/${
                    fromRequest().path(2)
                }/${fromRequest().path(3)}/${fromRequest().path(4)}",
                    "instance": "http://localhost/${fromRequest().path(0)}/${fromRequest().path(1)}/${
                    fromRequest().path(2)
